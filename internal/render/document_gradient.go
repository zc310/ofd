...
func newMeshGrid(triangles []ofdMeshTriangle) *meshGrid {
	...
	for _, triangle := range triangles {
		for _, point := range [3]canvas.Point{triangle.p0, triangle.p1, triangle.p2} {
...
func (g *ofdMeshGradient) At(x, y float64) color.RGBA {
	if value, ok := g.sampleAt(canvas.Point{X: x, Y: y}); ok {
		return value
	}
	if g.extend {
		return g.backColor
...
func (g *ofdMeshGradient) sampleAt(point canvas.Point) (color.RGBA, bool) {
...
func sampleMeshTriangle(triangle ofdMeshTriangle, point canvas.Point) (color.RGBA, bool) {
...
func sampleMeshTriangles(triangles []ofdMeshTriangle, point canvas.Point) (color.RGBA, bool) {
	for _, triangle := range triangles {
...
func newOFDLinearGradient(shd *models.CTAxialShd, transform func(models.StPos) canvas.Point, resolve colorResolver) canvas.Gradient {
...
func newOFDRadialGradient(shd *models.CTRadialShd, transform func(models.StPos) canvas.Point, resolve colorResolver) canvas.Gradient {
...
func newOFDGouraudGradient(shd *models.CTGouraudShd, transform func(models.StPos) canvas.Point, resolve colorResolver) canvas.Gradient {
	if shd == nil {
		return nil
	}
...
func newOFDLaGouraudGradient(shd *models.CTLaGouraudShd, transform func(models.StPos) canvas.Point, resolve colorResolver) canvas.Gradient {
...
func newGouraudVertex(point models.GouraudPoint, transform func(models.StPos) canvas.Point, resolve colorResolver) ofdMeshVertex {
...
func newLaGouraudVertex(point models.LaGouraudPoint, transform func(models.StPos) canvas.Point, resolve colorResolver) ofdMeshVertex {
	return ofdMeshVertex{
		point: transform(models.StPos{X: point.X, Y: point.Y}),
		color: meshColor(point.Color, resolve),
	}
...
func newMeshGradient(triangles []ofdMeshTriangle, backColor *models.CTColor, extend bool, resolve colorResolver) canvas.Gradient {
	if len(triangles) == 0 {
		return nil
	}
	for _, triangle := range triangles {
...
func addOFDGradientStops(gradient *canvas.Grad, segments []models.Segment, resolve colorResolver) {
	if len(segments) == 0 {
		return
	}
