// Package fgg 提供基于 fogleman/gg 的页面栅格化后端（fogleman API +
// freetype/raster，FloatTech/gg 的上游原版），通过空白导入注册到
// internal/render 的注册表（Go image 包风格）：
//
//	import _ "github.com/zc310/ofd/internal/render/backends/fgg"
//
// 之后即可用 drawing.BackendFGG="fgg" 作为栅格后端名，或经
// converter.RasterBackend("fgg") 使用。
...
func init() {
	_ = drawing.RegisterBackend(drawing.BackendFGG, func(width, height float64, resolution geom.Resolution) (drawing.Backend, error) {
		return New(width, height, resolution)
	})
}

// New 创建 fogleman/gg 光栅后端。输出分辨率和像素尺寸规则与 gogpu/gg
// 后端一致。
func New(width, height float64, resolution geom.Resolution) (drawing.Backend, error) {
	core, err := rastercore.New(width, height, resolution, func(wpx, hpx int, dpmm, hpix float64) rastercore.Hooks {
		...
		return &fggHooks{dc: gg.NewContextForRGBA(im), im: im, dpmm: dpmm, hpix: hpix}
	})
	if err != nil {
		return nil, err
	}
...
// fggHooks 是 rastercore.Hooks 的 fogleman/gg 实现。
//
// 已知差异（速度对照可接受）：
//   - freetype/raster 的 Stroke 对 nil joiner 回退为 RoundJoiner，因此 canvas
//     的 MiterJoin 在此降级为圆角连接（fogleman 无 Miter 连接器可用）。
//   - 渐变通过自定义 Pattern 逐像素采样 geom.Gradient，与 gogpu 的
//     CustomBrush 语义一致。
type fggHooks struct {
	dc   *gg.Context
	im   *image.RGBA
	dpmm float64
	hpix float64
...
func (h *fggHooks) Push() { h.dc.Push() }
func (h *fggHooks) Pop()  { h.dc.Pop() }

func (h *fggHooks) SetFillSolid(c color.Color) {
	h.dc.SetFillStyle(gg.NewSolidPattern(fggRGBA(c)))
}
func (h *fggHooks) SetStrokeSolid(c color.Color) {
	h.dc.SetStrokeStyle(gg.NewSolidPattern(fggRGBA(c)))
}
func (h *fggHooks) ClearFill()   { h.dc.SetFillStyle(gg.NewSolidPattern(color.Transparent)) }
func (h *fggHooks) ClearStroke() { h.dc.SetStrokeStyle(gg.NewSolidPattern(color.Transparent)) }

func (h *fggHooks) SetFillRule(rule geom.FillRule) {
	switch rule {
	case geom.NonZero:
		h.dc.SetFillRule(gg.FillRuleWinding)
	default:
		h.dc.SetFillRule(gg.FillRuleEvenOdd)
	}
}

func (h *fggHooks) DrawDevicePath(dp *rastercore.DevicePath, m geom.Matrix, fill, stroke bool, fillGrad, strokeGrad geom.Gradient, style rastercore.StrokeStyle) {
	replayPath(h.dc, dp)
	if fill {
		if fillGrad != nil {
			h.dc.SetFillStyle(&fggPattern{inv: m.Inv(), dpmm: h.dpmm, hpix: h.hpix, g: fillGrad})
		}
		// fogleman 的 Fill/Stroke 同样会清空路径：同时需要描边时用
	...
	if stroke {
		h.strokeStyle(style)
		if strokeGrad != nil {
			h.dc.SetStrokeStyle(&fggPattern{inv: m.Inv(), dpmm: h.dpmm, hpix: h.hpix, g: strokeGrad})
		}
		h.dc.Stroke()
	}
}

func (h *fggHooks) RenderImage(img image.Image, m geom.Matrix) {
	src := img
	margin := 0
...
func (h *fggHooks) Raster() *image.RGBA {
	dst := image.NewRGBA(h.im.Bounds())
	draw.Draw(dst, dst.Bounds(), h.im, h.im.Bounds().Min, draw.Src)
	return dst
}

// replayPath 把设备像素路径逐段追加到 fogleman 上下文的当前路径。
func replayPath(dc *gg.Context, dp *rastercore.DevicePath) {
...
// strokeStyle 把共享状态机换算好的描边造型写入 fogleman 上下文。虚线长度
// 与相位按线宽缩放（同 canvas 的 ScaleDash）；MiterJoin 无对应连接器，在
// freetype/raster 内回退为 RoundJoiner。
func (h *fggHooks) strokeStyle(style rastercore.StrokeStyle) {
...
func (p *fggPattern) ColorAt(x, y int) color.Color {
	pt := p.inv.Dot(geom.Point{X: float64(x) / p.dpmm, Y: (p.hpix - float64(y)) / p.dpmm})
	return p.g.At(pt.X, pt.Y)
}

// fggRGBA 把 color.Color 转换为直通 RGBA（与 canvas 默认 LinearColorSpace
// 一致，不做 gamma 往返）；fogleman 内部按 sRGB Over 合成。
func fggRGBA(c color.Color) color.RGBA {
	if c == nil {
		return color.RGBA{}
	}
	r, g, b, a := c.RGBA()
