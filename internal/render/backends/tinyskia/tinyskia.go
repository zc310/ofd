...
//
// 之后即可用 render.BackendTinySkia="tinyskia" 作为 Document.RasterizePage
// 的后端名。只用于渲染速度对照实验，不承诺与 canvas/gogpu 的逐像素输出
// 一致。渲染上下文状态机由 internal/render/backends/rastercore 共享。
package tinyskia
...
func init() {
	_ = render.RegisterBackend(render.BackendTinySkia, func(width, height float64, resolution canvas.Resolution) (render.Backend, error) {
...
// 恒等矩阵。渐变无法注入自定义采样器（Style 接口内部实现），改为把 canvas
...
func (h *tinyskiaHooks) SetFillRule(rule canvas.FillRule) { h.fillRule = rule }

func (h *tinyskiaHooks) DrawDevicePath(dp *rastercore.DevicePath, m canvas.Matrix, fill, stroke bool, fillGrad, strokeGrad canvas.Gradient, style rastercore.StrokeStyle) {
	h.ctx.BeginPath()
	replayPath(h.ctx, dp)
	if fill {
		if fillGrad != nil {
...
func (h *tinyskiaHooks) RenderImage(img image.Image, m canvas.Matrix) {
...
// setGradient 把 canvas 渐变几何换算到设备空间后重建为 tinyskia 渐变样式。
// isFill 选择 fill/stroke 样式。
func (h *tinyskiaHooks) setGradient(g canvas.Gradient, m canvas.Matrix, isFill bool) {
	set := h.ctx.SetFillStyleGradient
	if !isFill {
		set = h.ctx.SetStrokeStyleGradient
	}
	if lg, ok := g.(*canvas.LinearGradient); ok {
	...
	} else if rg, ok := g.(*canvas.RadialGradient); ok {
...
func (h *tinyskiaHooks) toDev(m canvas.Matrix, x, y float64) (float64, float64) {
...
// addStops 把 canvas 的渐变停靠点复制进 tinyskia 渐变对象。
func addStops(gr tinyskia.Gradient, grad canvas.Grad) {
	for _, s := range grad {
