...
Office 文档（doc/docx/odt/rtf/wps/pptx/xlsx 等）转 OFD/PDF 依赖 LibreOffice 命令行，支持范围见 [`docs/OFFICE-SUPPORT.md`](docs/OFFICE-SUPPORT.md)。

HTML/MHTML（html/htm/mhtml/mht）转 OFD/PDF 依赖 Chrome/Chromium（chromedp），支持范围见 [`docs/HTML-SUPPORT.md`](docs/HTML-SUPPORT.md)。
...
### PDF / Markdown / Office / HTML 文档转 OFD
...
Office 文档（doc/docx/odt/rtf/wps/pptx/xlsx 等）通过 LibreOffice 命令行转换：`Convert(from, "pdf", …)` 直接输出 PDF，`Convert(from, "ofd", …)` 先转 PDF 再生成 OFD。运行环境需安装 LibreOffice，否则返回明确错误；可用 `converter.WithSoffice(path)` 与 `converter.WithOfficeTimeout(d)` 指定可执行文件与超时。

HTML/MHTML（html/htm/mhtml/mht）通过 Chrome/Chromium 打印引擎转换，默认禁止加载外部资源，纸张默认 A4；可用 `converter.WithChrome(path)`、`converter.WithPaperSize("A4")`、`converter.WithLandscape(true)`、`converter.WithAllowRemoteResources(true)` 等选项控制。
