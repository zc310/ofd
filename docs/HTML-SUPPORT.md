...
| 格式                      | 当前状态 | 说明                                                |
|---------------------------|----------|-----------------------------------------------------|
| `.html`、`.htm`、`.xhtml` | 已支持   | 本地 HTML，含内联 CSS、`data:` 资源、本地相对资源。 |
| `.mhtml`、`.mht`          | 已支持   | MIME multipart 归档，内嵌资源由浏览器解析。         |

## 支持的输出格式

| 目标 | 当前状态 | 说明                                                            |
|------|----------|-----------------------------------------------------------------|
| PDF  | 已支持   | Chrome 直接输出；`converter.Convert(from, "pdf", …)`。          |
| OFD  | 已支持   | 先转 PDF，再经 `pdf2ofd`；文字可搜索，版式取决于 PDF 转换结果。 |
| 其他 | 已支持   | 先导入为 OFD 再走现有导出器（如 HTML→png）。                    |
